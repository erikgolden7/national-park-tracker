...
passport.use(
  new Auth0Strategy(
    ...
    function(accessToken, refreshToken, extraParams, profile, done) {
      const db = app.get('db');
      db.find_user([profile._json.sub]).then(user => {
        if (user[0]) {
          return done(null, {
            clientID: user[0].id,
            email: user[0].email,
          ...
          db.create_user([profile.displayName, profile.emails[0], profile.picture, profile.user_id]).then(user =>
            done(null, {
              clientID: user[0].id,
              email: user[0].email,
              name: user[0].full_name,
...
passport.deserializeUser(function(user, done) {
  app
    .get('db')
    .find_session_user([user.clientID])
    .then(user => {
